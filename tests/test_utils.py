...
class TestValidators:
    ...
    def test_validate_path_nonexistent_returns_path(self):
        """validate_path returns the path as-is even if it doesn't exist.
        This documents the current behaviour — existence check is left to callers.
        """
        from pathlib import Path
        result = validate_path("/this/path/does/not/exist/hopefully")
        assert result is not None  # returns path without raising
