...
[1] Guo, X., Okamura, H. & Dohi, T. (2024)
    ...
    Software Quality Journal 32, 543566
...
class EdgeCaseAnalyzer(BaseAnalyzer):
    ...
    def _generate_insights(self, metrics: Dict[str, MetricValue], functions: int) -> List[str]:
        ...
        if total_issues == 0:
            ...
            insights.append(f" Significant edge case gaps: {total_issues} issues found. Review recommended.")
        ...
        # Overflow risks
        if overflow > 5:
            insights.append(f" {overflow} numeric overflow risks found. Consider using Decimal or validation.")
        elif overflow > 0:
