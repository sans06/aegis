...
class GitHubRepoHandler(BaseInputHandler):
    ...
    def prepare(self) -> Path:
        ...
        try:
            ...
            url = self.url
            # FIX CR-02 / LV-13: NEVER embed token in URL.
            # Token-in-URL appears in .git/config (readable by all OS users),
            # process listings (ps aux), and git error messages on clone failure.
            # Confirmed 4 exposure vectors in Phase 4 logic validation.
            # Use GitPython's env parameter to pass credentials instead —
            # credentials in environment variables are not written to disk.
            # The actual env injection happens in the clone call below.
