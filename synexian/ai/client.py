...
from openai import AsyncOpenAI  # FIX CR-05: Use async client to avoid blocking event loop
...
class AIClient:
    ...
    def __init__(
        ...
        # FIX CR-05 / H-03: Use AsyncOpenAI instead of OpenAI.
        # The synchronous OpenAI client blocked the entire event loop for 3-15s
        # per AI call, defeating the parallelism of asyncio.gather() in the engine.
        # AsyncOpenAI lets other analyzers continue while awaiting API responses.
        ...
        # FIX C-04: Wire RateLimiter — previously implemented but never connected.
        # Without rate limiting, parallel AI calls all fired simultaneously,
        # hitting OpenRouter's rate limits (HTTP 429) on large projects.
        from synexian.ai.rate_limiter import TokenBucketRateLimiter
        # TokenBucketRateLimiter takes rate_per_second and burst_capacity
        # OpenRouter free tier: ~60 req/min = 1 req/sec, burst of 10
    ...
    async def complete(
        ...
        try:
            ...
            # FIX C-04: Acquire rate limit token before calling API
            await self._rate_limiter.acquire()

            # FIX CR-05: await the async client call (was previously blocking)
