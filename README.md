...
```bash
synexian analyze ./my-project --config custom-config.yaml
```
...
synexian analyze ./my-project --output cli,json,html
```

**JSON output only (for CI/CD):**

```bash
synexian analyze ./my-project --output json
```
...
synexian analyze <path> [OPTIONS]

# Configuration management
synexian config-show [--config PATH]
...
- name: Analyze Code Quality
  run: |
    pip install synexian-agent
    synexian analyze . --output json --config .synexian.yaml
